...
function getDayName(date) {
  ...
  return dayOfTheWeek[new Date(date).getDay()];
}
...
function getNextFriday(date) {
  ...
  const dayOfTheWeek = startDate.getUTCDay();
  const dayUntilFriday = (5 - dayOfTheWeek + 7) % 7 || 7;
  startDate.setUTCDate(startDate.getUTCDate() + dayUntilFriday);
  return startDate;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const difference = dateSecond.getTime() - dateFirst.getTime();
...
function isDateInPeriod(date, period) {
  ...
  return givenDate >= periodStart && givenDate <= periodEnd;
}
